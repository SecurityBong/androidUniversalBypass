...
console.log("[+] All enhancements applied successfully.");
    ...
    System.getProperty.implementation = function (key) {
        if (key === "ro.debuggable" || key === "ro.secure") {
            send("[+] Root detection bypassed for " + key);
            return "0";
        }
        return this.getProperty(key);
    };
    ...
    SecureSettings.getInt.overload("android.content.ContentResolver", "java.lang.String").implementation = function (resolver, name) {
        if (name === "adb_enabled") {
            send("[+] ADB detection bypassed");
            return 0;
        }
        return this.getInt(resolver, name);
    };

    // Native Library Hook: libpairipcore.so
    Interceptor.attach(Module.findExportByName("libpairipcore.so", "some_native_method"), {
        onEnter: function (args) {
            send("[+] Hooked native function in libpairipcore.so");
            send("    Arg0: " + args[0].toInt32());
        },
        onLeave: function (retval) {
            send("[+] Native function returned: " + retval.toInt32());
        }
    });

    // Memory Operations Hook (Fixing Pointer Misalignment)
    Interceptor.attach(Module.findExportByName(null, "memcpy"), {
        onEnter: function (args) {
            send("[+] memcpy called - realigning pointer");
            args[0] = ptr(args[0].toInt32() & ~0x3); // Ensure alignment
        }
    });

    Interceptor.attach(Module.findExportByName(null, "memmove"), {
        onEnter: function (args) {
            send("[+] memmove called - realigning pointer");
            args[0] = ptr(args[0].toInt32() & ~0x3);
        }
    });
    ...
    send("[+] All enhancements applied successfully.");
    ...
    // **PortSwigger Certificate Detection Bypass**
    try {
        // Hooking into methods used by PortSwigger to detect certificates
        safeUse('com.portswigger.package.CertificateDetection', function (CertificateDetection) {
            CertificateDetection.detectCertificates.implementation = function () {
                send('Bypassing PortSwigger Certificate Detection');
                return false;
            };
        });

        // Bypass any PortSwigger-specific SSL checks
        safeUse('com.portswigger.ssl.SSLCheck', function (SSLCheck) {
            SSLCheck.checkCertificate.implementation = function () {
                send('Bypassing PortSwigger SSL Certificate Check');
                return true; // Return true to simulate a valid certificate
            };
        });

    } catch (err) {
        send('Error hooking PortSwigger detection: ' + err);
    }
    ...
    try {
        ...
        function obfuscateMethodNames() {
            ...
            for (var i = 0; i < methods.length; i++) {
                ...
                send('Obfuscating method: ' + originalMethod + ' to ' + obfuscatedMethod);
            }
        }
        obfuscateMethodNames();

    } catch (err) {
        send('Obfuscation of PortSwigger methods failed, proceeding without obfuscation.');
    }
});
